...
def preprocess_train_test_dataset_for_alpha_3(
    run_number,
    dataset_csv_file,
    test_csv_file,
):
    ...
    df_train = df_dataset[
        ~df_dataset["split"].isin([split_idx, split_idx + 1])
    ]
    df_val = df_dataset[df_dataset["split"].isin([split_idx, split_idx + 1])]
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = (
        df_train["sentence_embedding"],
        df_train["class"],
        df_val["sentence_embedding"],
        df_val["class"],
        df_test["sentence_embedding"],
        df_test["class"],
    )
